% Methodology (draft: data / features / models / SCS / UI / risk control / evaluation protocol)
\label{sec:method}
...
\label{sec:ui}
Two distinct kinds of uncertainty are relevant at decision time.
\emph{Forecast (predictive) uncertainty} asks whether the model's own
probability estimate is trustworthy; the standard measures are
confidence ($1 - p_{\max}$), entropy $H(p) = -\sum_i p_i \log p_i$,
and proper-scoring-rule expectations such as $1 - \sum_i p_i^2$.
\emph{Decision (market) uncertainty} asks whether the betting
opportunity itself is stable: cross-bookmaker dispersion and
opening-to-closing odds movement measure how much the market
disagrees and how late information arrives. The objective is not to
estimate pure predictive uncertainty in a theoretically complete
sense. Instead, a \emph{decision-oriented} uncertainty indicator is
constructed that combines predictive ambiguity with market
instability:
\[
...
consistency (1 for deterministic models). The purpose is not to
recover a theoretically complete uncertainty distribution but to
construct an operational risk indicator for selective decision
making under market constraints. The shared market terms are
intentional: the UI inherits market uncertainty and adds the model's
own confidence signal. Table~\ref{tab:uicomponents} maps each
component to the quantity it is intended to measure.

\begin{table}[htbp]
\centering
\small
\caption{Components of the uncertainty index and the quantity each is
intended to measure. The first two terms quantify forecast
uncertainty, the last two decision uncertainty.}
\label{tab:uicomponents}
\resizebox{\linewidth}{!}{
\begin{tabular}{ll}
\toprule
...
$\mathrm{vol}_{norm}$ & market disagreement (cross-bookmaker dispersion) \\
$\mathrm{move}_{norm}$ & information instability (opening-to-closing drift) \\
\bottomrule
\end{tabular}
}
\end{table}

Why this formulation rather than a generic uncertainty estimator?
Entropy and $1 - \sum_i p_i^2$ are pure functions of the predicted
distribution: they re-encode confidence and, as
Section~\ref{sec:experiments} shows
(Table~\ref{tab:uncbaselines}), inherit its financial failure mode
when used for abstention. Predictive variance from sampling is
degenerate in this setting (multi-sample consistency is $\approx 1$,
Section~\ref{sec:llm_analysis}), and Bayesian posterior sampling is
not available for the tree ensembles used here. Conformal prediction
would provide coverage guarantees on a calibration set, but in
three-class problems it produces trivially large prediction sets at
any useful coverage, and the goal here is ranking-based abstention
rather than guaranteed coverage. The UI is therefore deliberately a
decision-oriented composite: it is not claimed to be a better
estimator of predictive uncertainty than entropy, but it is
designed---and shown in Section~\ref{sec:experiments}---to be a
better abstention score.

The consistency term is retained even though Section~\ref{sec:llm_analysis} shows it carries
...
coverage. ROI is evaluated as a \emph{decision-quality proxy} under
closing-market assumptions: bets are settled at Bet365 closing odds,
and no claim of deployable arbitrage is made---real-world execution
would add taxes, withdrawal fees, and slippage
(Section~\ref{sec:discussion}). Predictive metrics are accuracy,
macro-F1, log loss, Brier score, and expected calibration error
(ECE). Financial metrics are ROI, Sharpe ratio, maximum drawdown,
and win rate. All main quantities are reported with bootstrap 95\%
